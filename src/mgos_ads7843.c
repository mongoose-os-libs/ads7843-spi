 ...
 * @param x If true then the x position is read. If false then the y position is
 read
 * @return An 8 bit ADC value that changes with the position on the selected
 axis that the user touches.
           Given the size of displays that this library is used with, 8 bit
 resolution is enough and it's
...
static int8_t get_touch_xpos(bool x) {
  ...
  int8_t cmd_byte = 1 << START_REG_BIT | 0 << ADC_MODE_12BIT_REG_BIT |
                    0 << SINGLE_ENDED_ADC_REG_BIT |
                    0 << POWER_DOWN_MODE_REG_BIT;
  cmd_byte &= 0xff;
...
static void ads7843_irh(int pin, void *arg) {
  ...
  while (loop_count < 200) {
    // These values are ADC values with the top left of the screen in landcape
    // mode
...
bool mgos_ads7843_spi_init(void) {
  mgos_gpio_set_mode(mgos_sys_config_get_ads7843_irq_pin(),
                     MGOS_GPIO_MODE_INPUT);
  mgos_gpio_set_pull(mgos_sys_config_get_ads7843_irq_pin(), MGOS_GPIO_PULL_UP);
  mgos_gpio_set_int_handler(mgos_sys_config_get_ads7843_irq_pin(),
                            MGOS_GPIO_INT_EDGE_ANY, ads7843_irh, NULL);
  mgos_gpio_enable_int(mgos_sys_config_get_ads7843_irq_pin());
  ...
  LOG(LL_INFO, ("s_max_x=%d, s_max_y=%d, screen_orentation=%s", s_max_x,
                s_max_y, screen_orentation ? "PORTRAIT" : "LANDSCAPE"));
  LOG(LL_INFO, ("min_adc_x=%.f, max_adc_x=%.f, adc_x_range=%.f", min_adc_x,
                max_adc_x, adc_x_range));
  LOG(LL_INFO, ("min_adc_y=%.f, max_adc_y=%.f, adc_y_range=%.f", min_adc_y,
                max_adc_y, adc_y_range));
...
/**
 * @brief Set the handler function that will be called when touch down/up events
 *occur.
 **/
...
/**
 * @brief Set the dimensions of the display that the touch screen is connected
 *to
